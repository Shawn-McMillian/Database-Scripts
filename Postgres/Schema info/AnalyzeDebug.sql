...
*******************************************************************************
**
** Script Name: AnalyzeDebug
**
** Created By:  Shawn McMillian
**
** Description: Examine the dead tuples, costs and thresholds to see if Analyze is working
**
** Databases:   Common
**
** Revision History:
...
SELECT	relid,
		...
		to_char(n_mod_since_analyze::bigint,'FM999,999,999,999') AS ModifiedSince,
		analyze_factor AS ScaleFactor, 
		analyze_threshold AS Threshold,
		to_char(ROUND((reltuples + analyze_threshold) * analyze_factor),'FM999,999,999,999') AS AnalyzeAt, 
		c_analyze_factor as OrScaleFactor, --Table overide value if one exists
		c_analyze_threshold as OrThreshold, --Table overide value if one exists
...
FROM Results
--WHERE n_dead_tup >= ROUND((reltuples + analyze_threshold) * analyze_factor)
